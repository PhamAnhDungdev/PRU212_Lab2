...
public class BridgeController : MonoBehaviour
{
    ...
    public float offsetDistanceBridge = 0.5f;
    public float durationBridgeFall = .5f;
    ...
    void StartBuildingBridge()
    {
        ...
        // Lấy hướng của player nhưng loại bỏ trục Y
        ...
        // Hướng cầu dựng đứng (Quan trọng: giữ nguyên Vector3.up nếu muốn cầu dựng đứng)
    ...
    Vector3 GetPillarTopPosition(Transform pillar)
    {
        ...
        if (capsule != null)
        {
            float height = capsule.height * pillar.lossyScale.y; // Tính chiều cao thật sự của Capsule
            Vector3 topPosition = pillar.position + Vector3.up * (height / 2);
            return topPosition;
        }
        // Nếu không có CapsuleCollider, dùng cách khác
    ...
    IEnumerator RotateBridgeOverTime(Transform bridge, Vector3 pivot, Vector3 axis, float angle, float duration)
    {
        ...
        {
            float step = (angle / duration) * Time.deltaTime; // Mỗi frame xoay một chút
            bridge.transform.RotateAround(pivot, axis, step); // Xoay quanh điểm pivot

            currentAngle += step;
            elapsedTime += Time.deltaTime;

            if (currentAngle >= angle) break; // Dừng khi xoay đủ 90 độ
            yield return null;
        }

        // Đảm bảo cầu đúng góc cuối cùng
        bridge.transform.RotateAround(pivot, axis, angle - currentAngle);
        Debug.Log("Cầu đã bắt đầu rơi theo vật lý!");
    }
    ...
    void FinishBuildingBridge()
    {
        ...
        Vector3 playerForward = player.transform.forward;
        playerForward.y = 0; // Chỉ lấy hướng ngang
        playerForward.Normalize();

        // Tính trục xoay: cầu xoay quanh cạnh dưới
        Vector3 pivotPoint = bridgeStartPosition; // Điểm xoay là vị trí ban đầu của cầu
        Vector3 rotationAxis = Vector3.Cross(Vector3.up, playerForward); // Trục xoay theo hướng player
